...
```bash
# 如果是私有仓库，需要先登录
docker login registry.cn-shanghai.aliyuncs.com -u <你的用户名>

# 拉取镜像
docker pull registry.cn-shanghai.aliyuncs.com/<你的命名空间>/ai-travel-planner:latest

# 运行容器
docker run -d -p 3000:80 registry.cn-shanghai.aliyuncs.com/<你的命名空间>/ai-travel-planner:latest
```
